...
/**
 * list_insert_after() - Insert a node after the pos node
 * @pos: pointer to the position node which will the node will add immediate after it
 * @node: the node to be inserted
*/
static inline void list_insert_after(struct list_head *pos, struct list_head *node)
{
...
static inline void list_insert_before(struct list_head *pos, struct list_head *node)
{
...
#define list_spec_entry(head, type, member, offset)         \
    ({                                                      \
        __typeof__(head) temp = head;                       \
        for (int i = 0; i < offset; i++, temp = temp->next) \
            ;                                               \
        list_entry(temp, type, member);                     \
    })

/**
 * list_rand_entry() - get the random entry of the list
 * @head: pointer to the head of the list
 * @type: type of the entry containing the list node
 * @member: name of the list_head member variable in struct @type
 */
#define list_rand_entry(head, type, member, max_len)                       \
    ({                                                                     \
        __typeof__(max_len) offset = __typeof__(max_len) rand() % max_len; \
        list_spec_entry(head, type, member, offset);                       \
    })
